...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function toggleSeparadoresLetras(ocultar) {
    document.querySelectorAll('.separador-letra').forEach(function (el) {
      if (ocultar) {
        ...
        el.style.margin = '0px';
        el.style.display = 'none'; // <-- Adicione esta linha!
      } else {
        el.style.visibility = '';
        el.style.height = '';
        el.style.padding = '';
        el.style.margin = '';
        el.style.display = ''; // <-- E esta linha!
      }
    });
  }
  ...
  if (searchInput) {
    searchInput.addEventListener("input", function () {
      ...
      // Mostra ou oculta todas as letras conforme o input
      toggleSeparadoresLetras(searchValue.length > 0);

      // Exibe/oculta sublinks conforme pesquisa
      document.querySelectorAll(".submenu .sublink").forEach(function (link) {
      ...
      // Exibe todas as separador-letra, depois oculta só as que ficaram sem sublinks visíveis abaixo delas
      document.querySelectorAll(".separador-letra").forEach(function (sep) {
        sep.style.display = "";
        // Se não houver nenhum sublink visível até o próximo separador, oculta esse separador
        let next = sep.nextElementSibling;
        let foundVisible = false;
        while (next && !next.classList.contains("separador-letra")) {
          if (next.classList.contains("sublink") && next.style.display !== "none") {
            foundVisible = true;
            break;
          }
          next = next.nextElementSibling;
        }
        if (!foundVisible) sep.style.display = "none";
      });

      // Se limpou busca, restaura tudo
      if (!searchValue) {
        document.querySelectorAll(".submenu .sublink").forEach(function (link) {
          link.style.display = "";
        });
  ...
  sidebarOpen.addEventListener("click", function () {
    ...
    // Limpa o campo de pesquisa e botão de limpar busca
    if (searchInput) {
      searchInput.value = "";
      if (clearSearch) clearSearch.style.display = "none";
    }

    // Aguarda o menu aparecer, então reseta tudo!
    setTimeout(() => {
      resetarMenuLateral();
      // Garante a remoção de todos os estilos inline dos separadores/letra
      document.querySelectorAll(".separador-letra").forEach(function (el) {
        el.style.display = "";
        el.style.visibility = "";
        el.style.height = "";
      ...
      if (isParent) {
        ...
        if (isOpen) {
          ...
          if (!isOpen && submenu) {
            ...
            // >>>>>>> ADICIONE ISSO <<<<<<<<
            submenu.querySelectorAll(".separador-letra").forEach(function (el) {
              el.style.display = "";
              el.style.visibility = "";
              el.style.height = "";
              el.style.padding = "";
