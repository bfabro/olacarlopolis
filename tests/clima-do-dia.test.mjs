...
test("versões de site, painel e service worker avançam juntas", () => {
  assert.match(site, /Release do site v650/);
  assert.match(html, /style\.css\?v=501/);
  assert.match(html, /script\.js\?v=701/);
  assert.match(html, /Olá Carlópolis v523/);
  assert.match(panel, /numero: 786/);
  assert.match(panel, /label: "v793"/);
  assert.match(panel, /data: "2026-09-23"/);
  assert.match(panelHtml, /painel\.css\?v=492/);
  assert.match(panelHtml, /painel\.js\?v=723/);
  assert.match(worker, /2026-09-23-pesque-solte-v841/);
});
