...
test("v47 preserva o segredo do matinho, amplia o desembarque e contextualiza as piadas", () => {
  assert.match(source, /Esse matinho pode ser o da sorte!/);
  assert.doesNotMatch(source, /Aqui só aparecem peixes acima de 6 kg/);
  const boatAt = (x, y) => ({ zone: "tributario", mode: "explore", player: { x, y, inBoat: true } });
  const touchingEverySide = [
    boatAt(8.15 * 32, 10.5 * 32),
    boatAt(14.85 * 32, 10.5 * 32),
    boatAt(11.5 * 32, 9.15 * 32),
    boatAt(11.5 * 32, 11.85 * 32)
  ];
